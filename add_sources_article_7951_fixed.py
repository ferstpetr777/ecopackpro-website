...
class ZipLockSourcesAdderFixed:
    def __init__(self):
    ...
    def find_insertion_point(self, content):
        ...
        # Ищем конец блока "Мы предлагаем" - после </p> и перед <hr />
        we_offer_pos = content.find('Мы предлагаем:')
        if we_offer_pos != -1:
            # Ищем конец этого блока - ищем </p> после всех пунктов
            after_offer = content[we_offer_pos:]
            
            # Ищем последний </p> перед <hr />
            last_p_end = after_offer.rfind('</p>')
            if last_p_end != -1:
                # Найдем позицию после </p> и перед <hr />
                insertion_point = we_offer_pos + last_p_end + 4  # +4 для длины </p>
...
"""
    ...
    def add_sources_to_article(self, post_id):
        ...
        print(f"✅ Найдено место для вставки на позиции: {insertion_point}")
        
        # Показываем контекст вокруг точки вставки
        context = content[insertion_point-100:insertion_point+100]
        print(f"\nКонтекст вокруг точки вставки:")
        print("=" * 60)
        print(context)
        print("=" * 60)
...
def main():
    """Основная функция"""
    print("=" * 80)
    print("📚 ДОБАВЛЕНИЕ ИСТОЧНИКОВ В СТАТЬЮ 7951 (ИСПРАВЛЕННАЯ ВЕРСИЯ)")
    print("=" * 80)
    print("Статья: Zip lock пакеты с подвесом")
    print("=" * 80)
    
    adder = ZipLockSourcesAdderFixed()
