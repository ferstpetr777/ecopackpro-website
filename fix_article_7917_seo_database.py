...
class Article7917SEOUpdater:
    ...
    def update_yoast_meta(self, post_id, focus_keyword, meta_description):
        """Обновление мета данных Yoast SEO в базе данных"""
        connection = self.connect_to_database()
        if not connection:
            return False
        ...
        try:
            ...
            # Обновляем или создаем мета данные Yoast SEO
            meta_updates = [
                ('_yoast_wpseo_focuskw', focus_keyword),
                ('_yoast_wpseo_metadesc', meta_description),
                ('_yoast_wpseo_title', f"{focus_keyword}"),
                ('_yoast_wpseo_canonical', f"{self.wp_url}/{focus_keyword.replace(' ', '-').lower()}/")
            ]
            
            success_count = 0
            
            for meta_key, meta_value in meta_updates:
                # Проверяем, существует ли уже эта мета запись
                cursor.execute("""
                ...
                if cursor.rowcount > 0:
                    success_count += 1
                    print(f"✅ Обновлено: {meta_key}")
                else:
                    print(f"❌ Ошибка обновления: {meta_key}")
            
            connection.commit()
            
            if success_count == len(meta_updates):
                print(f"✅ Все мета данные Yoast SEO обновлены ({success_count}/{len(meta_updates)})")
                return True
            else:
                print(f"⚠️  Частично обновлено: {success_count}/{len(meta_updates)}")
                return False
    ...
    def fix_article_7917(self):
        ...
        print("🔧 ИСПРАВЛЕНИЕ СТАТЬИ 7917 - SEO ПАРАМЕТРЫ")
        print("=" * 60)
        ...
        # Обновляем мета данные Yoast SEO
        print(f"\n🔧 Обновление мета данных Yoast SEO...")
        meta_success = self.update_yoast_meta(post_id, focus_keyword, meta_description)
