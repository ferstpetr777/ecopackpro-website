...
setInterval(function(){let total=0;jQuery('.quantity input.qty').each(function(){total+=parseInt(jQuery(this).val())||0});jQuery('.w-cart-quantity').html(total>0?total:'0');jQuery('.w-cart').each(function(){jQuery(this)[total>0?'removeClass':'addClass']('empty')});console.log('Автообновление счетчика корзины:',total)},3000);
