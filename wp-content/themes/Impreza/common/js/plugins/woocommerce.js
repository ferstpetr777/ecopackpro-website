!( function( $, _undefined ) {
	...
	$.extend( WooCommerce.prototype, {
		...
		_updateCart: function() {
			...
			$.each( self.$cart, function( _, cart ) {
				...
				if ( miniCartAmount !== _undefined ) {
					...
					let total = 0;
					// Исправлено: ищем quantity inputs на всей странице, а не только внутри корзины
					$( '.quantity input.qty' ).each( function( _, input ) {
		...
		/**
		 * Автоматическое обновление индикатора корзины при загрузке страницы
		 */
		_autoUpdateCart: function() {
			let self = this;
			
			// Принудительно обновляем индикатор корзины при загрузке страницы
			setTimeout(function() {
				self._updateCart();
			}, 100);
	...
	// === АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ ИНДИКАТОРА КОРЗИНЫ ===
	// Вызываем обновление индикатора корзины с задержкой для полной загрузки DOM
	setTimeout(function() {
		if ( $us.woocommerce && typeof $us.woocommerce._updateCart === 'function' ) {
			$us.woocommerce._updateCart();
		}
	}, 100);
	...
	$( function() {
		...
		// === АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ ИНДИКАТОРА КОРЗИНЫ ===
		// Функция для принудительного обновления индикатора корзины
		function forceUpdateCartIndicator() {
			let total = 0;
			// Ищем все quantity inputs на странице
			$( '.quantity input.qty' ).each( function( _, input ) {
				let quantity = parseInt( $( input ).val() ) || 0;
				total += quantity;
			} );
			
			// Обновляем все индикаторы корзины на странице
			$( '.w-cart-quantity' ).html( total > 0 ? total : '0' );
			$( '.w-cart' ).each( function() {
				let $cartEl = $( this );
				$cartEl[ total > 0 ? 'removeClass' : 'addClass' ]( 'empty' );
			...
			console.log('Принудительное обновление индикатора корзины:', total);
		}
		
		// Вызываем принудительное обновление при загрузке страницы
		forceUpdateCartIndicator();
		
		// Дополнительное обновление через 500ms
		setTimeout(forceUpdateCartIndicator, 500);
		
		// Дополнительное обновление через 1 секунду
		setTimeout(forceUpdateCartIndicator, 1000);
		
		// Периодическое обновление каждые 2 секунды
		setInterval(forceUpdateCartIndicator, 2000);
		
		// === ОБНОВЛЕНИЕ ИНДИКАТОРА ЧЕРЕЗ WOOCOMMERCE СОБЫТИЯ ===
		// Обновляем индикатор при изменении корзины
		$(document.body).on('updated_cart_totals updated_wc_div', function() {
			forceUpdateCartIndicator();
		});
		
		// Обновляем индикатор при загрузке страницы корзины
		...
		// Обновляем индикатор при клике на кнопки +/-
		$(document).on('click', '.quantity button', function() {
			setTimeout(forceUpdateCartIndicator, 100);
		});
	} );

	// === АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ ИНДИКАТОРА КОРЗИНЫ (ВНЕ JQUERY READY) ===
	// Функция для принудительного обновления индикатора корзины
	function forceUpdateCartIndicator() {
		let total = 0;
		// Ищем все quantity inputs на странице
		jQuery( '.quantity input.qty' ).each( function( _, input ) {
			let quantity = parseInt( jQuery( input ).val() ) || 0;
			total += quantity;
		} );
		
		// Обновляем все индикаторы корзины на странице
		jQuery( '.w-cart-quantity' ).html( total > 0 ? total : '0' );
		jQuery( '.w-cart' ).each( function() {
			let $cartEl = jQuery( this );
			$cartEl[ total > 0 ? 'removeClass' : 'addClass' ]( 'empty' );
		});
	...
	// Используем DOMContentLoaded для гарантированной загрузки
...
// === БЕЗОПАСНОЕ ИСПРАВЛЕНИЕ СЧЕТЧИКА КОРЗИНЫ ===
// Принудительное обновление счетчика корзины каждые 3 секунды
setInterval(function() {
    let total = 0;
    ...
    jQuery('.w-cart-quantity').html(total > 0 ? total : '0');
    jQuery('.w-cart').each(function() {
        jQuery(this)[total > 0 ? 'removeClass' : 'addClass']('empty');
    });
    console.log('Автообновление счетчика корзины:', total);
}, 3000);
