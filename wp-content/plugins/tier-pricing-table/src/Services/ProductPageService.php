...
class ProductPageService {
	...
	public function __construct() {
		...
		if ( 'yes' === $this->getContainer()->getSettings()->get( 'display_summary', 'yes' ) ) {
			add_action( $this->getContainer()->getSettings()->get( 'summary_position_hook',
				'woocommerce_after_add_to_cart_button' ), array( $this, 'renderSummary' ), 10 );
		}
	...
	public function getVariationIdFromURL( $productId ) {
		...
		if ( ! $product || ! in_array( $product->get_type(),
				TierPricingTablePlugin::getSupportedVariableProductTypes() ) ) {
			return null;
		}
	...
	public function getVariationPricingTable() {
		...
		$verifyNonce = apply_filters( 'tiered_pricing_table/frontend/load_variation/verify_nonce', true, $nonce,
			'get_pricing_table' );
