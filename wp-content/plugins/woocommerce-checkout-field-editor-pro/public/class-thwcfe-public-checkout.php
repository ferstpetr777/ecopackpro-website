			...
			foreach($fieldset as $key => $field) {
				...
				if(isset($field['custom']) && $field['custom'] && isset($posted[$key])){
					if(isset($field['user_meta']) && $field['user_meta']){
						...
						if($type === 'file'){
							$value = $posted[$key];
							$value = THWCFE_Utils::sanitize_uploads($value);
						}else{
							$value  = $posted[$key];
		...
		foreach($checkout_fields as $fieldset_key => $fieldset){
			...
			foreach($fieldset as $key => $field) {
				...
				if(isset($field['custom']) && $field['custom'] && isset($field['order_meta']) && $field['order_meta']){
					$type = $field['type'];
					$value = false;
					$accept_zero = apply_filters( 'thwcfe_accept_value_zero',false);
					
					if(isset($posted[$key])){
						if($accept_zero && ($posted[$key] == 0)){
							$value = $posted[$key];
						}else{
							$value = !empty($posted[$key]) ? $posted[$key] : false;
						}
