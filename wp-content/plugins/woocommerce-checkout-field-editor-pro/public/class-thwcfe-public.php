...
****/
			...
			for( $i=0; $i < $count; $i++ ){
				...
				if(is_array($file_new) && isset($file_new['name'])){
					...
					if($uploaded && !isset($uploaded['error'])){
						...
						$upload_base_dir = $upload_base_dir.'/thwcfe_uploads';
						$uploaded['file'] = str_replace($upload_base_dir, '',$uploaded['file']);
	...
	public function ajax_remove_uploaded(){
		...
		if(isset($_POST) && isset($_POST['file']) && $_POST['file']){
			$wp_upload_dir = wp_get_upload_dir();
			$upload_base_dir = isset($wp_upload_dir['basedir']) ? $wp_upload_dir['basedir'] : '';
			$wcfe_upload_dir = $upload_base_dir . '/thwcfe_uploads';
			...
			$file = isset($_POST['file']) ? THWCFE_Utils::sanitize_file_path($_POST['file']) : '';
			$file = $wcfe_upload_dir . $file;
			...
			$result = false;
			if(file_exists($file)){
				$result = wp_delete_file_from_directory( $file, $wcfe_upload_dir );
			}
			//$file = $_POST['file'];
			//$result = unlink($file);
