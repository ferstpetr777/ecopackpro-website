...
class THWCFE_Utils {
	...
	**********************************************/
	...
	public static function sanitize_uploads($upload_data) {
		$cleaned = array();
		if($upload_data){
			$data_arr = json_decode($upload_data,true);
			if(!empty($data_arr) && is_array($data_arr)){
				foreach($data_arr as $key => $upload_item){
					$name = isset($upload_item['name']) ? sanitize_file_name($upload_item['name']) : '';
					$url = isset($upload_item['url']) ? sanitize_url($upload_item['url']): '';

					$file = isset($upload_item['file']) ? sanitize_url($upload_item['file']): '';
					$file = file_exists($file) ? $file : '';

					$type = isset($upload_item['type']) ? sanitize_text_field($upload_item['type']): '';

					$size = isset($upload_item['size']) ? $upload_item['size'] : '';
					$size = (int)$size;
					...
					$cleaned[sanitize_file_name($key)] = $new_item;
				}
			}
	...
	public static function sanitize_file_path($path) {
	    ...
	    // Remove all instances of single dots (.) and double dots (..) in the path
	    $path = preg_replace('#/\.?/#', '/', $path);   // Remove /./ or //
	    $path = preg_replace('#(/\.\./)#', '/', $path); // Remove /../

	    // Remove any remaining double dots at the start (../) or end (/..)
	    $path = preg_replace('#^(\.\./)+#', '', $path); // Remove leading ../
	    $path = preg_replace('#(/(\.\./)+)+$#', '', $path); // Remove trailing /../

	    // Prevent potential path traversal by collapsing multiple slashes to a single slash
