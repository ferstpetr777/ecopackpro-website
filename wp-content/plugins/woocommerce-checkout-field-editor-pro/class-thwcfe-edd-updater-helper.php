...
if ( ! class_exists( 'THWCFE_EDD_Updater_Helper' ) ) :
	...
	class THWCFE_EDD_Updater_Helper {
		...
		protected $sw_identifier;
		protected $license_data_key;
		protected $domain;
			...
			if ( ! empty( $notice ) ) {
				if ( is_multisite() ) {
					...
					*/
					...
					} elseif ( $enable_notification_sub_site ) {
						$this->show_admin_notice_content( $notice, 'admin_notice' );
					}
				} else {
					$this->show_admin_notice_content( $notice, 'admin_notice' );
		...
		public function plugin_update_message( $plugin_data, $response ) {
			...
			if ( isset( $plugin_data['update'] ) && $plugin_data['update'] && ( ! $license_key ) || ! ( ( 'valid' === $license_status ) || ( 'active' === $license_status ) ) ) {
				...
				printf( esc_html__( 'You need an active license key to get updates and receive support for %1$s. We recommend you to %2$srenew your subscription%3$s if you want to update to the latest version.', 'text-domain' ), esc_html( $this->product_name ), '<a href="' . esc_attr( $home_url ) . '" target="_blank">', '</a>' );
