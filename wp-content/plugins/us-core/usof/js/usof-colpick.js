! function( $, undefined ) {
	...
	function usofColorPicker( input, options ) {
		...
		self.$document
			...
			.on( 'change', '.usof-colpick input[name=usof-colpick-type]', function( e ) {
				self._stop( e );
		...
		self._timeout = $ush.timeout( function() {
			self.$input.select();
		}, 5 );
	}
	 ...
	 */
	var api_usofColorPicker = $.extend( usofColorPicker.prototype, {
		...
		rgbaToHex: function( rgba ) {
			return '#' + Object.values( $.isPlainObject( rgba ) ? rgba : {} )
				.slice( 0, 3 )
				.map( function( value ) { return value <= 255 ? ( '0' + parseInt( value, 10 ).toString( 16 ) ).slice( -2 ) : 'ff' } )
				.join( '' );
		},
