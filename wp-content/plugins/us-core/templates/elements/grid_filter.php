...
$_atts['class'] = 'w-filter for_grid state_desktop';
$_atts['class'] .= isset( $classes ) ? $classes : '';
$_atts['class'] .= ' layout_' . $layout;
$_atts['class'] .= ' items_' . count( $filter_items );

// Show filter items as Toggles
...
foreach ( $filter_items as $filter_item ) {
	...
	$item_atts = array(
		'class' => sprintf( 'w-filter-item type_%s', $type ),
	...
	if ( in_array( $type, array( 'checkbox', 'radio' ) ) AND ! empty( $item_values ) ) {
		...
		foreach ( $item_values as $item_value ) {
			...
			if ( $show_color_swatch AND property_exists( $item_value, 'color_swatch' ) ) {
				$color_preview_style = '';
				if ( $item_value->color_swatch ) {
					$color_preview_style = sprintf(
						'style="background:%s; color:%s;"',
						$item_value->color_swatch,
						us_get_contrast_bg_color( $item_value->color_swatch )
					);
				}
				$item_value_html .= sprintf( '<span class="w-color-swatch" title="%s" %s></span>', esc_attr( $item_value->name ), $color_preview_style );
			}
