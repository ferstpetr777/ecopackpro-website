...
if ( $type == 'carousel' ) {
	...
	// Disable infinite loop when nothing to rotate (to avoid creating item clones)
	if ( $items_count <= $columns ) {
		$carousel_loop = 0;
	}
	...
	$carousel_settings = array(
		...
		'carousel_fade' => $carousel_fade ? 1 : 0,
		'center' => $carousel_center ? 1 : 0,
		'dots' => $carousel_dots ? 1 : 0,
		'items' => $columns,
		'loop' => $carousel_loop ? 1 : 0,
		'nav' => $carousel_arrows ? 1 : 0,
		'slideby' => $carousel_slideby ? 'page' : '1',
		'smooth_play' => $carousel_autoplay_smooth ? 1 : 0,
