...
if ( ! function_exists( 'us_is_gradient' ) ) {
	...
	function us_is_gradient( $value ) {
		return (
			...
			OR preg_match( '/\s?var\(.*-grad\s?\)$/', $value )
		);
	}
}
...
if ( ! function_exists( 'us_get_contrast_bg_color' ) ) {
	/**
	 ...
	 */
	function us_get_contrast_bg_color( $color ) {
		if ( empty( $color ) ) {
			return '';
		}

		// Difine if the color is RGBA
...
if ( ! function_exists( 'us_get_current_id' ) ) {
	...
	function us_get_current_id() {
		...
		// Grid item ID
		global $us_grid_outputs_items, $us_grid_item_type, $us_grid_term, $us_grid_user_ID;
		if ( $us_grid_outputs_items ) {
			...
			if ( $us_grid_item_type == 'term' AND $us_grid_term instanceof WP_Term ) {
				...
				// Grid user ID
			} elseif ( $us_grid_item_type == 'user' ) {
				$current_id = $us_grid_user_ID;
...
if ( ! function_exists( 'us_get_current_meta_type' ) ) {
	...
	function us_get_current_meta_type() {
		...
		if ( $us_grid_outputs_items ) {
			return (string) apply_filters( 'us_get_current_meta_type', $us_grid_item_type ?? 'post' );
		}
	...
	function us_conditions_are_met( $conditions, $conditions_operator ) {
		...
		foreach( $conditions as $i => $condition ) {
			...
			if ( $condition_param == 'post_type' ) {
				...
				$user_roles = ( us_arr_path( $condition, 'user_source' ) == 'current_post_author' ) ? get_the_author_meta( 'roles' ) : wp_get_current_user()->roles;
				...
				// Term of the current taxonomy archive page
...
if ( ! function_exists( 'us_get_terms_for_autocomplete' ) ) {
	...
	function us_ajax_get_terms_for_autocomplete() {
		...
		if ( is_admin() ) {
			...
			$taxonomy = $_GET['taxonomy'] ?? (string) us_arr_path( $_GET, 'tax' );
