	...
	$.extend( prototype, $ush.mixinEvents, {
		...
		_elmDuplicate: function( id ) {
			...
			if ( self.isReloadElm( parentId ) ) {
				...
				self.reloadElmInPreview( self.getElmParentId( parentId ) );
				$usb.history.commitChange( newElmId, _CHANGED_ACTION_.CREATE );
	...
	$.extend( prototype, {
		...
		_updateColumnsLayout: function( rowId, layout ) {
			...
			if ( layout.indexOf( '-' ) > - 1 ) {
				...
				var customColumns = $.trim( layout ).split( _REGEXP_SPACE_ );
				newColumnsCount = 0;
			...
			if ( ! renderNeeded ) {
				...
				self.renderShortcode( 'renderShortcode', {
					data: {
		...
		reloadElmInPreview: function( elmId, callback ) {
			...
			let parentId = self.getElmParentId( elmId ),
				elmIndex = self.getElmIndex( elmId ),
