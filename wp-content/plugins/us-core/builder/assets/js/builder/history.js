...
! function( $, undefined ) {
	...
	$.extend( History.prototype, $ush.mixinEvents, {
		...
		setLatestShortcodeUpdate: function( data ) {
			_$temp._latestShortcodeUpdate = $ush.toPlainObject( data );
		},
		...
		commitChange: function( id, action, useThrottle, extData ) {
			...
			var saveDataToHistory = function() {
				 ...
				 */
				if ( _CHANGED_ACTION_.UPDATE === action && ! $.isEmptyObject( _$temp._latestShortcodeUpdate ) ) {
					$.extend( data, _$temp._latestShortcodeUpdate );
					_$temp._latestShortcodeUpdate = {};
				}
