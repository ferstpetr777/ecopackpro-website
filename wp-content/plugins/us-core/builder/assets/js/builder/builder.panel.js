		...
		_changeField: function( selectedElmId, usofField, _, _skipSave ) {
			...
			var _updateShortcode = function( _currentData ) {
				...
				var shortcodeObj = $usb.builder.parseShortcode( oldShortcode ),
					...
					hasDefaultValue = _currentData.usofField.getDefaultValue() === _currentData.value;

				// Attribute updates
				var atts = $usb.builder.parseAtts( shortcodeObj.atts );
				if ( isShortcodeContent || hasDefaultValue ) {
			...
			if ( _skipSave !== true && instructions === true && ! _currentData.isActiveRecoveryTask ) {
				...
				self.__updateShortcode_long( function() {
					...
					// Get element ID for reload
					var elmId = _currentData.id;
					if ( ! $usb.builder.isReloadElm( elmId ) ) {
						elmId = $usb.builder.getElmParentId( elmId);
					}

					// Reload element in preview
					$usb.builder.reloadElmInPreview( elmId, function() {
						$usb.builder.trigger( 'shortcodeChanged', _currentData ); // for react in extensions
					} );
				} );
			}
