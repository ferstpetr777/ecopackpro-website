...
! function( $, undefined ) {
	...
	$.extend( prototype, {
		...
		_normalizeJsoncss: function( options ) {
			...
			for ( var prop in options ) {
				 ...
				 */
				if ( /(^color|-color$)/.test( prop ) && self.colorIsVariable( value ) ) {
	...
	$.extend( prototype, {
		...
		isGradient: function( value ) {
			value += ''; // To string
			return value.indexOf( 'gradient' ) > -1 || /\s?var\(.*-grad\s?\)$/.test( value ); // for support var(*-grad);
		},
		 ...
		 */
		colorIsVariable: function( value ) {
			var value = $ush.toString( value );
			return (
		...
		_$events: {
			...
			onPreviewParamChange: function( targetId, instructions, value, fieldType, isResponsiveValue ) {
				...
				if ( fieldType === 'color' && self.colorIsVariable( value ) ) {
					value = self.getColorVariableValue( value );
				}
