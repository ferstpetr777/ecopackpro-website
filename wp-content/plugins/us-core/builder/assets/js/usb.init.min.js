...
return self};$usbcore.$removeClass=function(node,className){var self=this;if($ush.isNode(node)&&className){(''+className).split(_REGEXP_SPACE_).map(function(itemClassName){if(!itemClassName){return}
...
return self};$usbcore.$hasClass=function(node,className){var self=this;if($ush.isNode(node)&&className){var classList=(''+className).split(_REGEXP_SPACE_);for(var i in classList){className=''+classList[i];if(!className){continue}
if(self.indexOf(className,(''+node.className).split(_REGEXP_SPACE_))>-1){return!0}}}
...
return self};$usbcore.cache=function(namespace){var self=this;if(!$.isPlainObject(_$$cache)){_$$cache={}}
...
return _$$cache[namespace]};_window.$usbcore=$usbcore}(jQuery);!function($,undefined){var _history=history,_location=location,_undefined=undefined,_window=window;if(!_window.$usb){return}
_window.$ush=_window.$ush||{};_window.$usbcore=_window.$usbcore||{};const _RESERVED_PARAMS_=['post'];var _$locationHref=_location.href;var _$url=new URL(_$locationHref);function URLManager(){var self=this;_window.onpopstate=function(e){self.setUrl(_location);var state=$.isPlainObject(e.state)?$ush.clone(e.state):{};$.extend(state,self.getDataOfChange())
...
state=$.isPlainObject(state)?$ush.clone(state):{};$.extend(state,self.getDataOfChange());_history.pushState(state,'',self.buildHref());$usb.trigger('urlManager.changed',state);_$locationHref=_location.href}});$usb.urlManager=new URLManager}(jQuery);!function($,undefined){var _window=window;if(!_window.$usb){return}
_window.$ush=_window.$ush||{};_window.$usbcore=_window.$usbcore||{};function Notify(container){var self=this;self._events={close:self._close.bind(self)};$(function(){self.$container=$(container);$usb.$document.on('click','.usb_action_notification_close',self._events.close)})}
...
$notification.fadeOut('fast',function(){$notification.remove()})},closeAll:function(){$('.usb-notification',$usb.$body).fadeOut('fast',function(){$(this).remove()})}});$usb.notify=new Notify('.usb-notification')}(jQuery);!function($,undefined){var _window=window,_document=document;if(!_window.$usb){return}
...
else if(state.oldParams.action===_siteSettings){$('.usb_action_show_add_elms',self.$header).removeClass('hidden');$('.usb_action_go_to_back',self.$header).addClass('hidden')}}});$usb.panel=new Panel('#usb-panel')}(jQuery);!function($,undefined){var _window=window;var ceil=Math.ceil,max=Math.max;if(!_window.$usb){return}
...
self.$screen.removeClass('resizable');$usbcore.cache('temp').remove('resizeData')}});$usb.preview=new Preview('#usb-preview')}(jQuery);!function($,undefined){var _window=window,isPlainObject=$.isPlainObject,isEmptyObject=$.isEmptyObject;if(!_window.$usb){return}
_window.$ush=_window.$ush||{};function CSSGenerator(collections){if(!isPlainObject(collections)){return ''}
...
return result+''}
$usb.cssGenerator=CSSGenerator}(jQuery);!function($,undefined){var _window=window,isUndefined=$ush.isUndefined,isEmptyObject=$.isEmptyObject,isPlainObject=$.isPlainObject;if(!_window.$usb){return}
...
var parentId=self.getElmParentId(id),strShortcode=self.getElmShortcode(id)||'',newElmId;strShortcode=strShortcode.replace(/(\s?el_id="([^\"]+)")/gi,'').replace(/usbid="([^\"]+)"/gi,function(_,elmId){elmId=self.getSpareElmId(elmId);if(!newElmId){newElmId=elmId}
return 'usbid="'+elmId+'"'});if(!strShortcode||!newElmId)return;var index=0,siblingsIds=self.getElmSiblingsId(id)||[];for(var i in siblingsIds){if(siblingsIds[i]===id){index=++i;break}}
if(!self._addShortcodeToContent(parentId,index,strShortcode)){return}
if(self.isReloadElm(parentId)){self.reloadElmInPreview(parentId);$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else if(self.isReloadParentElm(newElmId)){self.reloadElmInPreview(self.getElmParentId(parentId));$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else{self.addElmToPreview(newElmId,index,parentId,_undefined,newElmId)}},_elmCopy:function(id){var self=this;if(!self.isValidId(id)){return}
var content=$ush.toString(self.getElmShortcode(id));$ush.copyTextToClipboard(content.replace(/\susbid="([^\"]+)"/gi,''));$ush.storage('usb').set('сlipboard',content)},_elmPaste:function(id){var self=this;if(!self.isValidId(id)){return}
var content=$ush.toString($ush.storage('usb').get('сlipboard'));if(!content){$usb.notify.add($usb.getTextTranslation('empty_clipboard'),_NOTIFY_TYPE_.INFO);return}
...
if(self.isReloadElm(parentId)){self.reloadElmInPreview(parentId);$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else if(self.isReloadParentElm(newElmId)){self.reloadElmInPreview(self.getElmParentId(parentId));$usb.history.commitChange(newElmId,_CHANGED_ACTION_.CREATE)}else{self.addElmToPreview(newElmId,index,parentId)}},_elmDelete:function(removeId){var self=this;if(!self.isValidId(removeId)){return}
...
$usb.notify.add($usb.getTextTranslation('page_updated'),_NOTIFY_TYPE_.SUCCESS);if(self._isReloadPreviewAfterSave&&(self.isPageFieldsChanged()||self.isPageMetaChanged())){self._isReloadPreviewAfterSave=!1;$usb.reloadPreview()}
...
return!1},modeIsValid:function(mode){var _availableModes=['unknown','editor','preview','drag:add','drag:move',];return mode&&$usbcore.indexOf(mode,_availableModes)>-1},isMode:function(){var self=this,args=arguments;for(var i in args){if(self.modeIsValid(args[i])&&_$mode===args[i])return!0}
return!1},setMode:function(mode){var self=this;if(mode&&self.modeIsValid(mode)&&mode!==_$mode){$usb.trigger('builder.modeChanged',_$mode=mode);return!0}
...
return result.join(' ')},isValidId:function(id){return id&&/^([\w\-]+):(\d+)(\|[a-z\-]+)?$/.test(id)},isRow:function(id){return this.getElmName(id)==='vc_row'},isRowInner:function(id){return this.getElmName(id)==='vc_row_inner'},isColumn:function(id){return/^vc_column(_inner)?$/.test(this.getElmName(id))},isOutsideMainContainer:function(id){var self=this;return $usbcore.indexOf(self.getElmName(id),$usb.config('elms_outside_main_container',[]))>-1},isMainContainer:function(id){return id&&id===this.mainContainer},isElmContainer:function(name){var self=this;name=self.isValidId(name)?self.getElmName(name):name;return $usb.config('shortcode.containers',[]).includes(name)},isRootElmContainer:function(name){var self=this;name=self.isValidId(name)?self.getElmName(name):name;return(self.isElmContainer(name)&&!!$usb.config('shortcode.relations.as_parent.'+name+'.only'))},isChildElmContainer:function(name){var self=this;name=self.isValidId(name)?self.getElmName(name):name;return(self.isElmContainer(name)&&!self.isRootElmContainer(name)&&!!$usb.config('shortcode.relations.as_child.'+name+'.only'))},isReloadElm:function(elmId){var self=this;if($ush.isNode(elmId)){elmId=self.getElmId(elmId)}
if(!self.isValidId(elmId)){return!1}
return $usb.config('shortcode.reload_element',[]).includes(self.getElmName(elmId))},isReloadParentElm:function(elmId){var self=this;if($ush.isNode(elmId)){elmId=self.getElmId(elmId)}
if(!self.isValidId(elmId)){return!1}
return $usb.config('shortcode.reload_parent_element',[]).includes(self.getElmName(elmId))},isElmTTA:function(name){var self=this;if(self.isValidId(name)){name=self.getElmType(name)}
...
return callback.call(self)},renderShortcode:function(requestId,settings){var self=this;if(!requestId||$.isEmptyObject(settings)){return}
...
var elmId=self.getSpareElmId(type),elmName=self.getElmName(elmId),insert=self.getInsertPosition(parentId,elmIndex);if(!$.isPlainObject(values)){values={}}
...
if(!self._addShortcodeToContent(parentId,elmIndex,buildShortcode)){return!1}
if(self.isReloadElm(parentId)){self.reloadElmInPreview(parentId);$usb.history.commitChange(elmId,_CHANGED_ACTION_.CREATE)}else{self.addElmToPreview(elmId,elmIndex,parentId,callback)}
return elmId},moveElm:function(moveId,newParentId,newIndex){var self=this,args=arguments;if(self.isMainContainer(moveId)){$usb.log('Error: Cannot move the container',args);return!1}
var isMainContainer=self.isMainContainer(newParentId);if(self.hasSameTypeParent(moveId,newParentId)){$usb.log('Error: It is forbidden to add inside the same type',args);return}
if(!self.isValidId(moveId)||!(self.isValidId(newParentId)||isMainContainer)){$usb.log('Error: Invalid ID specified',args);return!1}
if(!self.doesElmExist(moveId)||!(self.doesElmExist(newParentId)||isMainContainer)){$usb.log('Error: Element doesn\'t exist',args);return!1}
var oldParentId=self.getElmParentId(moveId);newIndex=$ush.parseInt(newIndex);$usb.postMessage('doAction','hideHighlight');if(!isMainContainer&&!self.doesElmExist(newParentId)){newParentId=self.mainContainer;newIndex=0}
$usb.history.commitChange(moveId,_CHANGED_ACTION_.MOVE);var oldShortcode=self.getElmShortcode(moveId);self.pageData.content=$ush.toString(self.pageData.content).replace(oldShortcode,'');var insert=self.getInsertPosition(newParentId,newIndex);if(!self._addShortcodeToContent(newParentId,newIndex,oldShortcode)){return!1}
...
return match});$usb.builder.renderShortcode('_renderPastedContent',{data:{content:pastedContent,isReturnContent:!0,},success:function(res){if(!res.success||!res.data.html){return}
...
var name=$usb.builder.getElmName(id);if(!name){return}
var values=$usb.builder.getElmValues(id)||{},defaultValues=$usb.config('shortcode.default_values.'+name,{});if($.isPlainObject(defaultValues)){for(var k in defaultValues)if(k!=='content'){if($ush.isUndefined(values[k])){values[k]=defaultValues[k]}}}
...
var shortcodeObj=$usb.builder.parseShortcode(oldShortcode),isShortcodeContent=(_currentData.fieldType==='editor'||_currentData.name==='content'),hasDefaultValue=_currentData.usofField.getDefaultValue()===_currentData.value;var atts=$usb.builder.parseAtts(shortcodeObj.atts);if(isShortcodeContent||hasDefaultValue){delete atts[_currentData.name]}
else{atts[_currentData.name]=_currentData.value}
shortcodeObj.atts=$usb.builder.buildAtts(atts);if(isShortcodeContent){shortcodeObj.content=_currentData.value}
var newShortcode=$usb.builder.buildShortcode(shortcodeObj),hasChanged=(oldShortcode!==newShortcode&&!_currentData.isActiveRecoveryTask),oldParentShortcode;if(_currentData.instructions===!0&&$usb.builder.isReloadParentElm(originalId)){_currentData.id=$usb.builder.getElmParentId(_currentData.id);oldParentShortcode=$usb.builder.getElmShortcode(_currentData.id)}
...
if(hasChanged){$usb.history.setLatestShortcodeUpdate({content:oldShortcode,preview:$usb.builder.getElmOuterHtml(_currentData.id)});var commitArgs=[_currentData.id,_CHANGED_ACTION_.UPDATE];commitArgs.push($usb.config('useThrottleForFields',[]).indexOf(_currentData.usofField.type)>-1);if(oldParentShortcode){commitArgs.push({originalId:originalId})}
...
cache.flush();var elmId=_currentData.id;if(!$usb.builder.isReloadElm(elmId)){elmId=$usb.builder.getElmParentId(elmId)}
$usb.builder.reloadElmInPreview(elmId,function(){$usb.builder.trigger('shortcodeChanged',_currentData)})})}
...
if(urlManager.hasParam('navigator','show')){self.show()}else{self.hide()}}});$usb.navigator=new Navigator('#usb-navigator')}(jQuery);!function($,undefined){var _window=window,_document=document;if(!_window.$usb){return}
...
var insert=$usb.builder.getInsertPosition(parentId,currentIndex);var _getTemplateData=function(){$usb.postMessage('showPreloader',[insert.parent,insert.position,]);$usb.builder.renderShortcode('templates.getTemplateData',{data:{template_category_id:categoryId,template_id:templateId,isReturnContent:!0},success:function(res){$usb.postMessage('hidePreloader',insert.parent);if(!res.success||!res.data.content||!res.data.html){return}
...
var self=this,name=usofField.name;if($usb.builder.pageData.fields[name]===value){return}
$usb.builder.pageData.fields[name]=value;if(name==='post_title'){_document.title=$usb.config('adminPageTitleMask',value).replace('%s',value);$usb.postMessage('updateElmContent',['.post_title,head > title',value,'text'])}
if(name==='thumbnail_id'&&(usofField.$row.data('usb-params')||{}).usb_preview===!0){$usb.builder._isReloadPreviewAfterSave=!0}
$ush.debounce_fn_1ms(function(){$usb.trigger('builder.contentChange')})},_changePageMeta:function(usofField,value){if(!(usofField instanceof $usof.field)){return}
var self=this,name=usofField.name;if($usb.builder.pageData.pageMeta[name]===value){return}
...
if(_CHANGED_ACTION_.UPDATE===action&&!$.isEmptyObject(_$temp._latestShortcodeUpdate)){$.extend(data,_$temp._latestShortcodeUpdate);_$temp._latestShortcodeUpdate={}}
...
self.trigger('historyChanged');$ush.timeout(self._recoveryTaskManager.bind(self),1)}});$usb.history=new History}(jQuery);!function($,undefined){var _window=window;if(!_window.$usb){return}
_window.$ush=_window.$ush||{};_window.$usbcore=_window.$usbcore||{};var _$temp={fieldsets:{},isProcessSave:!1,savedLiveOptions:{},};function SiteSettings(container){var self=this;self.selectedGroupId;self.activeFieldset;self.$activeFieldset;if($usb.urlManager.hasParam('action',$usb.config('actions.site_settings'))){_window.onbeforeunload=function(e){$usb.postMessage('clearCookies')}}
