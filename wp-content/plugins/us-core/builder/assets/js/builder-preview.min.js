...
if(!($ush.isFirefox||$ush.safariVersion()>=17)){self._events.stop(e)}
...
var negativeTop=elm.offsetTop<0?abs(elm.offsetTop):0,negativeLeft=elm.offsetLeft<0?abs(elm.offsetLeft):0,elmRect=$ush.$rect(elm),cssProps={height:elmRect.height-negativeTop,left:elmRect.left+negativeLeft+(_window.pageXOffset||elm.scrollLeft),top:elmRect.top+negativeTop+(_window.pageYOffset||elm.scrollTop),width:elmRect.width-negativeLeft};$(item.highlight).css(cssProps);$usbcore.$toggleClass(item.highlight,'small',(elmRect.width<100))}},showEditableHighlight:function(id){var self=this;if(!$usb.builder.isValidId(id)){return}
...
if($usb.builder.isValidId(self.selectedElmId)&&$usb.builder.getElmType(self.selectedElmId)!=='us_grid'){self.showEditableHighlight(self.selectedElmId);self.setHighlightsPosition()}},_mouseLeavesIframe:function(e){var self=this;if($usb.panel.isShow()){self.hideHighlight()}},_elmSelected:function(e){var self=this;if(!$usb.builder.isMode('editor')||!$usb.panel.isShow()||$usbcore.$hasClass(e.target,'usb_skip_elmSelected')){return}
...
self._toggleDesignSpecificClasses.apply(self,arguments)},_removeDesignForElmsNotExist:function(){var self=this;_document.querySelectorAll('style[data-for]').forEach(function(style){var id=$usbcore.$attr(style,'data-for');if(id&&null===self.getElmNode(id)){$usbcore.$remove(style)}})},_removeDesignById:function(id){_document.querySelectorAll('style[data-for="'+id+'"]').forEach(function(style){$usbcore.$remove(style)})},setDesignStyles:function(className,jsoncss){var self=this,collections={};jsoncss=$ush.toPlainObject(jsoncss);if($.isEmptyObject(jsoncss)){return ''}
$usb.config('responsiveStates',[]).map(function(screen){if(!!jsoncss[screen]){collections[screen]=self._normalizeJsoncss(jsoncss[screen])}});var result='';for(var screen in collections){if($.isEmptyObject(collections[screen])){continue}
...
var value=options[prop];if(/(^color|-color$)/.test(prop)&&self.colorIsVariable(value)){value=self.getColorVariableValue(value);if(prop=='color'&&self.isGradient(value)){value=value.replace('-grad','')}
options[prop]=value}
...
return options},_buildBackground:function(collection){var self=this;collection=$ush.toPlainObject(collection);var names=['color','image','repeat','attachment','position','size'],props={},background='';for(var i in names){var name=names[i],propName='background-'+name;if(collection[propName]){props[name]=collection[propName];delete collection[propName]}}
var colorGradient='';if(props.image&&self.isGradient(props.color)){colorGradient=props.color;delete props.color}
...
collection.background=background.trim();return collection},_toggleDesignSpecificClasses:function(id,jsoncss,specificClasses){var self=this,toggleClasses={};if(!$.isPlainObject(specificClasses)){return toggleClasses}
...
var nodeRect=$ush.$rect(node),offsetTop=(nodeRect.x+_window.scrollX),offsetLeft=(nodeRect.y+_window.scrollY);return(pageX>=offsetTop&&pageX<=(offsetTop+nodeRect.width)&&pageY>=offsetLeft&&pageY<=(offsetLeft+nodeRect.height))},isGradient:function(value){value+='';return value.indexOf('gradient')>-1||/\s?var\(.*-grad\s?\)$/.test(value)},colorIsVariable:function(value){var value=$ush.toString(value);return(/^_([\dA-z\-_]+)$/.test(value)||/^{{([\dA-z\/\|\-_]+)}}$/.test(value))},getColorVariableValue:function(value){if(this.colorIsVariable(value)){return $ush.toString($usof.getData('colorVars')[value]||value)}
return value},_getNearestNode:function(node,asContainer){var self=this,args=arguments,foundId;if(!$ush.isNode(node)){return null}
...
return _filter(parentId,recursionLevel++)};var foundId=_filter(foundId)||$usb.builder.mainContainer;return self.getElmNode(foundId)},_getTargetElm:function(targetId,position){var self=this;if(!targetId||!$usb||$usbcore.indexOf(position,['before','prepend','append','after'])<0){return}
...
if(!isMainContainer&&$targetElm.length){var elmRootSelector=$usb.config('rootContainerSelectors.'+$usb.builder.getElmName(targetId));if(elmRootSelector){$targetElm=$(''+elmRootSelector,$targetElm).first();if(!$targetElm.length){$usb.log('Error: No element set for container `%s` in rootContainerSelectors'.replace('%s',elmType))}}}
...
return ''},tabIsHidden:function(id){var self=this;return $usb.builder.isElmSection(id)&&!$('.w-tabs-section-content:first',self.getElmNode(id)).is(':visible')},openSectionById:function(id){var self=this;if(self.tabIsHidden(id)){$('.w-tabs-section-header:first',self.getElmNode(id)).trigger('click')}},scrollToOutsideElm:function(id){var self=this;if(!$usb.builder.isValidId(id)){return}
...
if(fieldType==='color'&&self.colorIsVariable(value)){value=self.getColorVariableValue(value)}
