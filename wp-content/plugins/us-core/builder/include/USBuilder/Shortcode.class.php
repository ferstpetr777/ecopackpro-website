...
final class USBuilder_Shortcode {
	...
	/**
	 * Content for $usb.pageData.content
	 */
	private $page_data_content;
	...
	function set_page_content() {
		...
		// If the current call is in a loop get the content from the first post only, to avoid issue #4147
		if ( in_the_loop() AND did_action( 'the_post' ) !== 1 ) {
			return;
		}

		global $post;
		if ( $post instanceof WP_Post ) {
			$this->page_data_content = $this->prepare_text( $post->post_content );
		}
	}
	...
	function export_page_data() {
		...
		echo '<script id="usb-content" type="text/post_content">' . $this->page_data_content .'</script><script>' . $jscode . '</script>';
