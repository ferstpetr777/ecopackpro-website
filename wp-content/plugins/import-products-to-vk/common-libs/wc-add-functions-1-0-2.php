...
if ( ! function_exists( 'the_cat_tree' ) ) {
	 ...
	 * 
	 * @since 1.0.2 (15-03-2024)
	 *
	 * @param string $term_name - Optional
	 * @param int $term_id - Optional
	 * @param array $value_arr - Optional - id выбранных ранее глобальных атрибутов
	 * @param string $separator - Optional
	 * @param bool $parent_shown - Optional
	 * 
	 * @return string
	 */
	function the_cat_tree( $term_name = '', $term_id = -1, $value_arr = [], $separator = '', $parent_shown = true ) {
		...
		if ( $parent_shown ) {
			...
			$result = sprintf(
				...
				$term->name,
				$term->term_id,
				__( 'products', 'import-products-to-vk' ),
				$term->count,
				$selected,
				$separator,
				$term->name
		...
		$separator .= '-';
		$args_arr['parent'] = $term_id;
		$terms = get_terms( $args_arr );
		if ( count( $terms ) > 0 ) {
...
if ( ! function_exists( 'wooс_delete_product' ) ) {
	 ...
	 * @since 1.0.0 (23-05-2023)
	 * 
	 * @param int $id - Required - the product ID
	 * @param bool $force - Ooptional - true to permanently delete product, false to move to trash
	 *
